...
class FifaClient(object):
	...
	def __init__(self, username, password, securityHash, authData):
		...
		self.authData = self.data = json.dumps(authData, separators=(',',':'))
		self.start = 9000
		self.doLogin()
	...
	def doLogin(self):
		...
		requestor = UrlRequestor("https://www.ea.com/uk/football/services/authenticate/login", {'Content-Type': 'application/x-www-form-urlencoded'}, content)
		requestor.open()
